...
echo '{'
first=1
for BRANCH in 202211 202305 202311 202405 202411 202505 master
do
	if [[ -z "${first}" ]]; then
		echo ','
	fi
	...
	BUILD_INVM="$(curl -s 'https://dev.azure.com/mssonic/build/_apis/build/builds?definitions='"${DEFID_INVM}"'&branchName=refs/heads/'"${BRANCH}"'&$top=1&resultFilter=succeeded&api-version=6.0' | jq -r '.value[0].id')"
	...
	if [[ "${BUILD_INVM}" != "null" ]]; then
		echo "\"sonic-innovium.bin\": {"
		echo "  \"url\": \"$(echo "${ARTF_INVM}" | sed 's/format=zip/format=file\&subpath=\/target\/sonic-innovium.bin/')\","
